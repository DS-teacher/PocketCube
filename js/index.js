(function ($, window, document, undefined) {
  ...
  var positionParts = function (turnAxis, level, direction) {
    ...
    $.each(elements, function (i, el) {
      $.each(axis, function (j, ax) {
        $(el).attr(
          "data-" + ax,
          data[turnAxis][oppositeDirection][level][i][ax]
        );
      });
    });
  };
  ...
  var performTurn = function (rotate3d, turnAxis, direction, callback) {
    ...
    $(".grip")
      .find(".axis-grip:first")
      .css({
        transform: "rotate3d(" + rotate3d[turnAxis] + degree + "deg)",
      })
  ...
  var rotateParts = function (rotate3d, turnAxis, direction) {
    $(".grip .cube-part-wrapper").each(function (i, el) {
      ...
      css = css === "none" ? "" : css;
      css = "rotate3d(" + rotate3d[turnAxis] + degree + "deg) " + css;
  ...
  var moveSwiper = function (e) {
    if (canvas.swiper && canvas.swiper.play) {
      ...
      canvas.swiper.distance.x =
        ((canvas.swiper.start.x - canvas.swiper.move.x) /
          canvas.swiper.size.x) *
        90;
      canvas.swiper.distance.y =
        ((canvas.swiper.start.y - canvas.swiper.move.y) /
          canvas.swiper.size.x) *
        90;

      if (
        !canvas.swiper.active &&
      ...
      if (canvas.swiper.axis) {
        if (!canvas.swiper.init) {
          ...
          if (canvas.swiper.delta[canvas.swiper.side] < 1.5) {
            ...
            canvas.swiper.level =
              canvas.swiper.side === "y" && canvas.swiper.control === "right"
                ? 0
                : canvas.swiper.level;
            canvas.swiper.level =
              canvas.swiper.side === "x" && canvas.swiper.control === "front"
                ? 2
                : canvas.swiper.level;
            canvas.swiper.level =
              canvas.swiper.side === "x" && canvas.swiper.control === "bottom"
                ? 2
                : canvas.swiper.level;

            canvas.swiper.level =
              canvas.swiper.controls === "front" && canvas.swiper.side === "x"
                ? 0
                : canvas.swiper.level;
        ...
        factor =
          canvas.swiper.side === "x" && canvas.swiper.control === "bottom"
            ...
            : factor;
        canvas.swiper.degree =
          ((canvas.swiper.start[canvas.swiper.turn] -
            canvas.swiper.move[canvas.swiper.turn]) /
            canvas.swiper.size[canvas.swiper.side]) *
          90 *
          factor;
        $gripAxis.css({
          transform:
  ...
  var moveCube = function (e) {
    ...
    var transformation = "";
    if (
      (x > canvas.threshold || x < -1 * canvas.threshold) &&
      canvas["axis"] !== "x"
    ) {
      canvas["axis"] = "y";
      ...
      (y > canvas.threshold || y < -1 * canvas.threshold) &&
      canvas["axis"] !== "y"
    ) {
      canvas["axis"] = "x";
  ...
  var releaseCube = function (e) {
    if (canvas.down) {
      ...
      canvas["mouse"]["time"] =
        canvas["mouse"]["end"] - canvas["mouse"]["start"];
      canvas["mouse"]["bouce"] =
        (canvas["mouse"]["distance"] / canvas["mouse"]["time"]) * 100;
      canvas["css"] = $(".cube-wrapper")[0].style.transform;
      canvas["mouse"]["bouce"] =
        Math.abs(canvas["mouse"]["bouce"]) <= 5 ? 0 : canvas["mouse"]["bouce"];
      $(".cube-wrapper")
        .css({
          transitionDuration:
            Math.min(Math.abs(canvas["mouse"]["bouce"] * 20), 300) + "ms",
          transform:
            "rotate3d(" +
            rotate3d[canvas["axis"]] +
            canvas["mouse"]["bouce"] * cubeRotationSpeed +
            "deg) " +
            canvas["css"],
        })
        .one("transitionend", function () {
  ...
  var onMouseDown = function (e) {
    e.preventDefault();
    if ($(e.target).is(".swiper") || $(e.target).closest(".controls").length) {
      return false;
    }
    canvas.down = true;
    canvas["axis"] = false;
